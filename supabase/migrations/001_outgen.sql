-- OutGen — run once in Supabase SQL Editor (Dashboard → SQL → New query)

-- Profiles (1 welcome credit on signup)
...
-- New user → profile with 1 complimentary credit
...
-- Atomic print order + credit deduction
...
BEGIN
  ...
  IF p_maps_url IS NULL OR length(trim(p_maps_url)) < 10 THEN
...
-- Add credits (subscription refresh or purchase — callable by authenticated user for demo)
...
BEGIN
  ...
  INSERT INTO public.credit_transactions (user_id, delta, reason)
  VALUES (v_user_id, 3, 'monthly_subscription');
